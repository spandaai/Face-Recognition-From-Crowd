...
class Predictor:
    ...
    def predict(self, image):
        ...
        print("Prediction shape before argmax:", prediction.shape)
        if prediction.shape[1] == 0:
            raise ValueError("Model output has no elements along dimension 1")
...
class Trainer:
    def __init__(self, root_dir, label_map, embeddings, pretrained=True):
    ...
    def ssl_trainer(self):
        ...
        data = SSLFacentDataModule(
            root_dir=self.root_dir, label_map=self.label_map, batch_size=16
        )
        data.setup()
...
if __name__ == "__main__":
    ...
    face_trainer = Trainer(
        root_dir=utils.ROOT_PATH + "/database/faces",

    ...
    def run(self):
        ...
        if self.args.source == "live":
            ...
            self.out = cv2.VideoWriter(
                self.args.temp_dir + "output.avi", fourcc, 20.0, (640, 480)
            )
            for frame in Process(
                os.path.abspath("./temp"),
                "./weights/yolov7-tinyface.pt",
                self.args.path,
            ).start_capture():
            ...
            for frame in Process(
                os.path.abspath(self.args.path),
                "./weights/yolov7-tinyface.pt",
                self.args.path,
                crop_faces=True if self.args.crop_face else False,
            ).start_capture():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", type=str, default="live", help="live || video/image path"
    )
